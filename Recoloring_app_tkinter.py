...
class Recoloring_app:
	...
	def __init__(self, controller):
		...
		self.newWindow.geometry("750x400")
		...
		self.var1 = IntVar()
		Checkbutton(self.newWindow, text="Local (mask-based)", variable=self.var1, command=self.update_txt1).grid(row=4, sticky=W, padx=120, pady=10)

		self.var2 = IntVar()
		Checkbutton(self.newWindow, text="Local (mouse-based) ", variable=self.var2, command=self.update_txt2).grid(row=4, sticky=W, padx=270, pady=10)

		self.scribble_size=IntVar()
		Label(self.newWindow, text="Scribble_size").grid(row=4,sticky=W, padx=480, pady=10)
		self.scribble_size=Scale(self.newWindow, from_=2, to=30, orient=HORIZONTAL)
		self.scribble_size.grid(row=4, sticky=W, padx=600, pady=10)
		self.scribble_size.set(30)


		Label(self.newWindow, text="Existing_pigments_color:").grid(row=5,sticky=W, pady=10)
		...
		for i in range(len(self.Existing_rgb)):
		    ...
		    l.grid(row=5, sticky=W, padx=180+20*i, pady=10)
		...
		Button(self.newWindow, text='Save palette', command=self.save_palette).grid(row=14, sticky=W, padx=360, pady=15)

		Button(self.newWindow, text='Quit', command=self.Quit).grid(row=14, sticky=W, padx=540, pady=15)
